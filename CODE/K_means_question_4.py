...
emotions = {'High': 0, 'Neutral': 1, 'Low': 2}
for item in image:
    # getting label corresponding to the image
    a = data['Answer.Q4Answer'].loc[data['Input.image_url'] == 'https://lijingyang.me/images/AmazonMTurk/' + item.split('/')[-1]]
    try:
...
print(classification_report(testY, model.predict(testX), target_names=['High', 'Neutral', 'Low']))
