...
emotions = {'Happy': 0, 'Sad': 0, 'Anger': 1, 'Neutral': 0, 'Surprise': 0, 'Disgust': 0, 'Scared': 0, 'Not clear': 0}
for item in image:
...
# print('[INFO] {} images deleted'.format(i))
print('[INFO] shape of images is', images.shape)
print('[INFO] shape of labels is', labels.shape)

(trainX, testX, trainY, testY) = train_test_split(images, labels, test_size=0.05)
...
H = model.fit_generator(aug.flow(trainX, trainY, batch_size=32), validation_data=(testX, testY), steps_per_epoch=len(trainX) // 32, epochs=20, verbose=1)
